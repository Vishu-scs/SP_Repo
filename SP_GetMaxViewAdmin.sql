...
LEFT JOIN norms..Userfeedback ufb on pm.partid = ufb.partid 
...
WHERE sn.stockdate = DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
   AND(
		 @Status IS NULL  
        OR (@Status = 0 AND (ufb.UserFBStatus = ''Pending'' OR ufb.UserFBStatus IS NULL)) 
         OR (@Status = 1 AND ufb.UserFBStatus = ''Approved'')
   )
    AND (
...
END;
GO
--EXEC GetMAXData @DealerID = 8 , @r1 = null ,@r2 = null,@PartNumber = null,@LocationID = 14,@MaxValueFlag = null,@seasonalid = null,@natureid = null,@modelid = null,@Status = null;
--EXEC GetMAXDataAdmin   20230,null,null,null,40857,null,null,null,null,null;
